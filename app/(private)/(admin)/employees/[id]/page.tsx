...
export default function EmployeeDetailPage() {
    ...
    const fetchEmployeeData = async () => {
      try {
        ...
        if (data.success) {
          ...
          if (data.data.performance_history && data.data.performance_history.length > 0) {
  ...
  const getStatusBadge = (status: boolean) => {
    return status ? <Badge variant="default">Active</Badge> : <Badge variant="destructive">Inactive</Badge>
  }
  ...
  const currentDepartment = employee.department_history.find(dept => dept.is_active) || employee.department_history[0]
  // Get latest performance review
  const latestPerformance = employee.performance_history.length > 0 ? employee.performance_history[0] : null
            ...
            <Card className="lg:col-span-1">
              <CardHeader className="text-center">
                <Avatar className="h-20 w-20 sm:h-24 sm:w-24 mx-auto">
                  <AvatarImage src="/placeholder.svg" alt={`${employee.personal_details.first_name} ${employee.personal_details.last_name}`} />
                ...
                <CardDescription className="text-sm">{currentDepartment?.designation}</CardDescription>
                {getStatusBadge(currentDepartment?.is_active ?? false)}
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex items-center gap-2 text-sm">
                  <Mail className="h-4 w-4 text-muted-foreground shrink-0" />
                  <span className="truncate">{employee.personal_details.email}</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                ...
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="truncate">{employee.personal_details.current_address}</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                  <CalendarDays className="h-4 w-4 text-muted-foreground shrink-0" />
                  <span>Joined {new Date(employee.personal_details.created_at).toLocaleDateString()}</span>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="space-y-6">
                <div className="grid gap-4 grid-cols-1 sm:grid-cols-2">
                  <div>
                    ...
                    <p className="text-sm font-medium">{currentDepartment?.department_name}</p>
                  </div>
                  <div>
                    <h4 className="text-sm font-medium text-muted-foreground">Employee ID</h4>
                    <p className="text-sm font-medium">{employee.personal_details.employee_id}</p>
                  </div>
                  <div>
                    <h4 className="text-sm font-medium text-muted-foreground">Date of Birth</h4>
                    <p className="text-sm font-medium">
                      {new Date(employee.personal_details.date_of_birth).toLocaleDateString()}
                    </p>
                  </div>
                  <div>
                    <h4 className="text-sm font-medium text-muted-foreground">Marital Status</h4>
                    <p className="text-sm font-medium">{employee.personal_details.marital_status}</p>
                  </div>
                  <div>
                    <h4 className="text-sm font-medium text-muted-foreground">Emergency Contact</h4>
                    <p className="text-sm font-medium">
                      {employee.personal_details.emergency_contact_name} ({employee.personal_details.emergency_contact_phone})
                    </p>
                  </div>
                  <div>
                    <h4 className="text-sm font-medium text-muted-foreground">Blood Group</h4>
                    <p className="text-sm font-medium">{employee.personal_details.blood_group}</p>
                  </div>
                </div>
                <Separator />
                <div>
                  <h4 className="text-sm font-medium text-muted-foreground mb-2">Permanent Address</h4>
                  <p className="text-sm">{employee.personal_details.permanent_address}</p>
                </div>
              </CardContent>
            </Card>
          </div>
        ...
        <TabsContent value="performance" className="space-y-4 sm:space-y-6">
          <div className="grid gap-4 sm:gap-6 grid-cols-1 lg:grid-cols-2">
            {latestPerformance ? (
              <Card>
                <CardHeader>
                  <CardTitle className="text-lg sm:text-xl">Latest Performance Review</CardTitle>
                  <CardDescription className="text-sm">
                    Review from {new Date(latestPerformance.review_date).toLocaleDateString()}
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-4">
                      <Progress value={parseFloat(latestPerformance.performance_score)} className="h-2 flex-1" />
                      <span className="font-medium">{latestPerformance.performance_score}%</span>
                    </div>
                  </div>
                  <div className="grid gap-4 grid-cols-1 sm:grid-cols-2">
                    <div>
                      <h4 className="text-sm font-medium text-muted-foreground">Key Strengths</h4>
                      <p className="text-sm">{latestPerformance.key_strengths}</p>
                    </div>
                    <div>
                      <h4 className="text-sm font-medium text-muted-foreground">Areas for Improvement</h4>
                      <p className="text-sm">{latestPerformance.areas_for_improvement}</p>
                    </div>
                  </div>
                  <Separator />
                  <div>
                  ...
                  <div className="grid gap-4 grid-cols-1 sm:grid-cols-2">
                    <div>
                      <h4 className="text-sm font-medium text-muted-foreground">Bonus Awarded</h4>
                      <p className="text-sm">NPR {latestPerformance.bonus_awarded}</p>
                    </div>
                    <div>
                      <h4 className="text-sm font-medium text-muted-foreground">Promotion Eligible</h4>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-lg sm:text-xl">Performance Review</CardTitle>
                  <CardDescription className="text-sm">No performance reviews available</CardDescription>
                </CardHeader>
                <CardContent>
